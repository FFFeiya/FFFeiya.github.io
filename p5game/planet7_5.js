...
function getModelGfx(key, w, h) {
  let gfx = modelGfxCache.get(key);
  if (!gfx || gfx.width !== w || gfx.height !== h) {
    gfx = createGraphics(w, h, WEBGL);
    modelGfxCache.set(key, gfx);
  }
  return gfx;
}

function renderModelPreview(gfx, modelToRender, labelWhenMissing) {
  gfx.background(255);
  gfx.stroke(0);
  gfx.noFill();
  gfx.push();
  gfx.rotateY(frameCount * 0.01);
  if (gfx.width >= 200) gfx.scale(1.5);
  else gfx.scale(1.0);
  if (modelToRender) {
    gfx.model(modelToRender);
  } else {
    gfx.resetMatrix();
    gfx.fill(0);
    gfx.noStroke();
    gfx.textAlign(CENTER, CENTER);
    gfx.textSize(gfx.width >= 200 ? 24 : 16);
    gfx.text(labelWhenMissing || getText('loading'), 0, 0);
  }
  gfx.pop();
}
...
class ShipSelectionModal {
  ...
  display() {
    ...
    if (this.selectedShip) {
      ...
      let gfx = getModelGfx(`shipModal_${this.selectedShip.name}`, 140, 140);
      let modelToRender = shipModels[this.selectedShip.name] || defaultModel;
      renderModelPreview(gfx, modelToRender, getText('loading'));
      image(gfx, detailX + 205, detailY + 55, 140, 140);
    } else {
      fill(0);
      textSize(16);
      textAlign(CENTER, CENTER);
...
class EquipmentSelectionModal {
  ...
  display() {
    ...
    if (this.selectedEquip) {
      ...
      stroke(0);
      rect(detailX + 200, detailY + 50, 150, 150);
      let gfx = getModelGfx(`equipModal_${this.selectedEquip.name}`, 140, 140);
...
function drawSelectedShipPreviewFleet() {
  ...
  if (gameState === 'fleetFormation' && currentModal instanceof ShipSelectionModal) {
    ...
    if (selectedShip) {
      ...
      let gfx = getModelGfx(`fleetPreview_${selectedShip.name}`, 140, 140);
      let modelToRender = shipModels[selectedShip.name] || defaultModel;
      renderModelPreview(gfx, modelToRender, getText('loading'));
      image(gfx, -70, -70, 140, 140);
      pop();
    }
  }
}
...
function drawSelectedShipPreview() {
  ...
  if (gameState === 'battle' && currentModal === null) {
    ...
    if (selectedShip && selectedShip.health > 0) {
      ...
      // 创建一个p5.Graphics对象用于渲染3D模型
      let gfx = getModelGfx(`battlePreview_${selectedShip.name}`, 260, 260);
      let modelToRender = shipModels[selectedShip.name] || defaultModel;
      renderModelPreview(gfx, modelToRender, getText('loading'));
      image(gfx, 0, 0, 260, 260); // 调整模型显示位置
      pop();
    }
  }
}
